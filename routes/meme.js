  ...
  .post(async (req, res) => {
    ...
    MemeApi.createMeme(params)
      .then(async (el) => {
        ...
        console.log(oneMeme.name, img, oneMeme.width, oneMeme.height, oneMeme.box_count, totalText)
        const newMeme = await Meme.create({name: oneMeme.name, url: img, width: oneMeme.width, height: oneMeme.height, box_count : oneMeme.box_count, text: totalText, owner: userId })
